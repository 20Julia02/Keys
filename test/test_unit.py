...
def test_create_unapproved_device(mock_db):
    unapproved_service = deviceService.UnapprovedDeviceService(mock_db)
    unapproved_data = {"device_id": 2, "is_taken": False, "issue_return_session_id": 1}
...
def test_get_unapproved_dev_session(mock_db):
    unapproved_service = deviceService.UnapprovedDeviceService(mock_db)
    mock_device = models.DeviceUnapproved(device_id=3)
...
def test_transfer_devices(mock_db):
    unapproved_service = deviceService.UnapprovedDeviceService(mock_db)
    mock_unapproved = models.DeviceUnapproved(device_id=4, is_taken=True, last_taken="2024-01-01")
    mock_device = models.Device(code=4)
