...
def test_get_unauthorized_user_email(db: Session, 
                                     ...
                                     concierge_token: str):
    """
    Test the /email endpoint to retrieve an unauthorized user by email.
    """
    ...
    assert data["name"] == unauthorized_user.name
    assert data["surname"] == unauthorized_user.surname
...
#     assert response.status_code == 404
#     assert response.json()["detail"] == "No device notes found"
...
#     assert response.status_code == 404
#     assert response.json()[
#         "detail"] == f"No device notes found"
