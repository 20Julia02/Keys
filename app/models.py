...
class BaseUser(Base):
    ...
    sessions: Mapped[List["IssueReturnSession"]
...
class User(BaseUser):
    ...
    sessions: Mapped[List["IssueReturnSession"]
                     ] = relationship(back_populates="concierge")
...
class IssueReturnSession(Base):
    ...
    id: Mapped[intpk]
    user_id: Mapped[Optional[int]] = mapped_column(ForeignKey("base_user.id"))
    concierge_id: Mapped[Optional[int]] = mapped_column(ForeignKey("user.id"))
